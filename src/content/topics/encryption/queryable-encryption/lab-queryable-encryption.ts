  ...
  steps: [
    {
      ...
      instructions: 'Create a Node.js script (createQEDeks.cjs) that creates two separate DEKs - one for salary and one for taxId. Use keyAltNames "qe-salary-dek" and "qe-taxid-dek" respectively. Use Run all or Run selection in the editor to run the script and generate the keys.',
    ...
    },
    {
      ...
      instructions: 'Create the collection with encryptedFields configuration using the provided Node.js script. Define salary and taxId as encrypted fields with their respective query types. Use Run all or Run selection in the editor to run the script.',
    ...
    },
    {
      ...
      instructions: 'Create a QE-enabled client using autoEncryption and encryptedFieldsMap, then insert at least 3–5 test documents into the hr.employees collection with different salary values and taxId strings. Use Run all or Run selection in the editor to run the insert script.',
    ...
    },
    {
      ...
      instructions: 'Write a script that first queries hr.employees with a standard MongoClient (showing Binary ciphertext and failed equality queries), then connects with a QE-enabled client using encryptedFieldsMap and successfully runs equality queries on salary and taxId. Use Run all or Run selection in the editor to run the script.',
      estimatedTimeMinutes: 15,
      modes: ['demo', 'lab'],
      verificationId: 'qe.verifyQERangeQuery',
      points: 20,
