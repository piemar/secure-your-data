...
export const lab3Definition: WorkshopLabDefinition = {
  ...
  steps: [
    ...
    },
    {
      ...
      instructions: 'Create DEKs for multiple tenants, each with a unique keyAltName (e.g., "tenant-1-ssn-key", "tenant-2-ssn-key"). Insert encrypted data for each tenant using their respective DEKs. Use Run all or Run selection in the editor to run the script.',
      estimatedTimeMinutes: 15,
      modes: ['demo', 'lab', 'challenge'],
      verificationId: 'csfle.verifyMultiTenantKeys',
      points: 20,
      enhancementId: 'right-to-erasure.multi-tenant-keys',
      preview: { type: 'encryption-demo', config: { variant: 'crypto-shredding' } },
      hints: [
    ...
    },
    {
      ...
      instructions: 'Create a script that locates the DEK used for SSN encryption by keyAltName, then uses ClientEncryption.rewrapManyDataKey() to rewrap that DEK with a (simulated) new CMK alias. Use Run all or Run selection in the editor to run the script.',
      estimatedTimeMinutes: 15,
      modes: ['lab', 'challenge'],
      verificationId: 'csfle.verifyKeyRotation',
      points: 20,
      enhancementId: 'right-to-erasure.key-rotation',
      preview: { type: 'encryption-demo', config: { variant: 'key-rotation' } },
      hints: [
