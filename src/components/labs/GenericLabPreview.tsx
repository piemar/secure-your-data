  ...
  if (!hasRun) {
    return (
      <div className="rounded border border-dashed border-border bg-muted/30 p-4 text-center text-sm text-muted-foreground">
        ...
        <p>Run the code to see encrypted vs decrypted view.</p>
      </div>
    );
  }

  return (
    <div className="rounded border border-border bg-card p-3 text-xs space-y-2">
      <p className="text-muted-foreground">
        Mode: {mode}. Fields: {fields.join(', ')}.
      </p>
...
export function GenericLabPreview({
  ...
  const content = useMemo(() => {
    ...
    if (preview.type === 'encryption-demo') {
      return <EncryptionDemoPreviewRenderer config={preview.config} hasRun={hasRun} />;
    }
