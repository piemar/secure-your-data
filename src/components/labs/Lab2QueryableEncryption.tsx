...
run().catch(console.error);`,
          skeleton: `// ══════════════════════════════════════════════════════════════
// Query Encrypted Data - QE vs Standard Client Comparison
// ══════════════════════════════════════════════════════════════
// Compare how encrypted data appears with and without QE.
...
async function run() {
  ...
  // TASK: Query with standard client - encrypted fields show as Binary
  const docStandard = await standardCollection._______({ name: "Alice Johnson" });
  console.log("Salary (encrypted):", docStandard.salary); // Binary(...)
  await clientStandard.close();
  ...
  // TASK: Define encryptedFields for the QE client
  const encryptedFields = {
    fields: [
      { path: "salary", bsonType: "int", keyId: salaryDekId, queries: { queryType: "________" } },
      { path: "taxId", bsonType: "string", keyId: taxDekId, queries: { queryType: "equality" } }
    ]
  };

  // TASK: Create QE-enabled MongoClient with autoEncryption
  const clientQE = new MongoClient(uri, {
    _______________: {
      keyVaultNamespace: "encryption.__keyVault",
      kmsProviders,
  ...
  // TASK: Query with QE client - fields are automatically decrypted
  const docQE = await qeCollection.findOne({ name: "Alice Johnson" });
  console.log("Salary (decrypted):", docQE.______); // 75000
  console.log("✅ Fields are automatically decrypted!");

  // TASK: Execute equality query on encrypted taxId field
  const equalityResult = await qeCollection.findOne({ _____: "123-45-6789" });
...
run().catch(console.error);`,
          // Inline hints for Step 4 - line numbers match skeleton positions
          inlineHints: [
            { line: 30, blankText: '_______', hint: 'Method to find a single document', answer: 'findOne' },
            { line: 46, blankText: '____________', hint: 'The keyAltName for the taxId DEK', answer: 'qe-taxid-dek' },
            { line: 55, blankText: '________', hint: 'Query type for searching encrypted salary', answer: 'equality' },
            { line: 61, blankText: '_______________', hint: 'Config property to enable automatic encryption', answer: 'autoEncryption' },
            { line: 73, blankText: '______', hint: 'The property name for salary data', answer: 'salary' },
            { line: 77, blankText: '_____', hint: 'The field name for tax identification', answer: 'taxId' }
          ]
        },
        {
          filename: 'Terminal - Run the Node.js script',
