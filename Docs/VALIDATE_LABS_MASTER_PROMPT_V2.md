# Master Template Prompt: Validate Existing Labs Against ADD_LAB_MASTER_PROMPT (V2 – with prompt improvement feedback)

This is the **V2** version of the VALIDATE_LABS master prompt. It adds an optional **prompt improvement feedback** step at the end of each run so missing criteria or ambiguities can be suggested and later folded into the prompts. See **Prompt improvement feedback (V2)** below and **`Docs/PROMPT_IMPROVEMENTS_LOG.md`** for the log and process.
...
| **Prerequisites** | Concrete list (Atlas M10+, tools, etc.). **When the lab has any step with a Mongosh block:** prerequisites must include **mongosh** (or "MongoDB Shell") so attendees know Run requires mongosh installed and path set in Workshop Settings; otherwise "mongosh missing" or Run failures occur. **When the lab has any step with a C# block:** prerequisites should mention **.NET SDK 10.0+** (e.g. "Optional: .NET SDK 10.0+ to run the C# tab") so attendees can run C# blocks. | `prerequisites` present and non-empty. If any step's enhancement has a block with `language: 'mongosh'`, check that prerequisites mention mongosh; if missing, list as gap. If any step has a block with `language: 'csharp'`, check that prerequisites mention .NET SDK; if missing, note as optional gap. |
...
| **Inline hints vs skeleton** | Every `inlineHints` entry: `line` must be a valid 1-based line in `skeleton`, and that line must contain the exact `blankText` (so the "?" marker can be placed correctly). **At most one placeholder and one hint per row/code line:** no two `inlineHints` may share the same `line`; no skeleton line may contain more than one placeholder. **No orphaned hints:** if a block has `skeleton` but **no placeholders** (no run of 2+ underscores), then `inlineHints` must be **empty or omitted**. **No placeholders without hints:** if a block has placeholders in `skeleton`, **every** placeholder must have exactly one matching `inlineHint` (same `line` and `blankText`). Applies to **all** blocks including Mongosh and C#. | Run **hint rendering validation test** (see below). Check: (1) skeleton with no placeholders → inlineHints empty/omitted; (2) every placeholder has one hint; (3) at most one hint per line. List any block that has hints with no placeholders or placeholders with no hint. |
...
| **C# blocks** | C# blocks must have `language: 'csharp'`, filename ending in `.cs`, and the same rigour as Node/Mongosh: `skeleton` and `inlineHints` when the step has fill-in-the-blank. **Baseline:** When a step has both mongosh (or Node) and C#, the C# block must implement the **same logic** as the mongosh/Node block (convert from mongosh/Node to C# driver). **MongoDB C# driver:** Use sync API; `InsertOne`/`InsertMany` do not return inserted IDs—solution code must read `doc["_id"]` and (for many) `docs.Select(d => d["_id"].ToString())` after the call, not rely on a return value. Use `BsonDocument`, `BsonArray`, `MongoClient`, `GetDatabase`, `GetCollection<BsonDocument>`, `Environment.GetEnvironmentVariable("MONGODB_URI")`. **Testing:** For each step with a C# block, the full solution code should be runnable (e.g. via `scripts/test-run-csharp.mjs` or `/api/run-csharp` with the step's solution code). | If a step has a C# block without skeleton/inlineHints where the step has guided blanks, list as gap. If C# solution code uses a return value from `InsertOne`/`InsertMany` (driver returns void), list as "Fix C# code: read _id from document(s) after insert." If C# block logic does not match the mongosh/Node block for the same step, list as "Align C# block with mongosh/Node baseline." |
| **Tips** | **2–4 tips** per enhancement. Tips should mention Run all / Run selection where execution is from the editor (no separate terminal). **For enhancements that include a Mongosh block:** at least one tip should mention that Run requires mongosh to be installed and its path set in Workshop Settings (e.g. "Run uses the mongosh path from Workshop Settings; set it if Run fails or you see mongosh missing."). **For enhancements that include a C# block:** optionally mention that the C# tab requires .NET SDK 10.0+ and that output appears in the terminal (when Terminal tab is used) or in the Run panel. | Count `tips?.length`; if &lt; 2 or missing, list as gap. If tips still say "run in terminal" for node/mongosh steps, list as "Update tips: use Run all or Run selection." If enhancement has a Mongosh block but no tip mentions mongosh path / Workshop Settings, list as "Add tip: Run requires mongosh path in Workshop Settings." |
...
**Placeholders and hint markers must be correct:** For every step that uses a skeleton with fill-in-the-blank placeholders, validation must confirm that (1) **no orphaned hints:** blocks whose skeleton has **no** placeholders (no `_________` / underscore run) have **no** `inlineHints` (empty or omitted); (2) **no placeholders without hints:** every placeholder in the skeleton has exactly one matching `inlineHint` (correct `line` and `blankText`); (3) each hint’s `line` and `blankText` match the skeleton (so the "?" marker can be placed); (4) at most one placeholder and one hint per line; (5) visual check: the "?" marker aligns with the placeholder. Run the hint rendering test and perform a visual check so placeholders and hint markers are correct.
...
**Prompt improvement feedback (optional, V2):** After producing the fix plan, if this audit revealed a **criterion that was missing** from this prompt (or from ADD_LAB_MASTER_PROMPT), or an **ambiguity** that led to inconsistent findings, append a short **Prompt improvement suggestion** to your response with: (1) **Issue:** What was wrong or unclear. (2) **Suggested change:** Exact addition or edit for the prompt doc (one or two sentences or a bullet). (3) **Where:** Section name or criterion in VALIDATE_LABS_MASTER_PROMPT or ADD_LAB_MASTER_PROMPT. Only suggest if the same gap would likely affect future audits. Do not suggest style-only tweaks. If nothing critical, omit this section. The maintainer can add the suggestion to Docs/PROMPT_IMPROVEMENTS_LOG.md and later apply it to the prompts.
```
...
When using this V2 prompt, the AI may append an optional **Prompt improvement suggestion** at the end of its response (see the Master prompt block). If you receive one, add a row to **`Docs/PROMPT_IMPROVEMENTS_LOG.md`** with status **pending**. Periodically review pending suggestions, edit `VALIDATE_LABS_MASTER_PROMPT.md` or `VALIDATE_LABS_MASTER_PROMPT_V2.md` (and ADD_LAB if relevant) as needed, then mark the row as **applied** or **rejected**. This keeps the prompts improving over time without automatic self-editing.
