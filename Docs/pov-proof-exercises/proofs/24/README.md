...
* Add filters by clicking the _pencil_ below the toggle to expand the __Audit Filter Settings__ and the perform the following in the Filter Builder that appears:
  * Click __USE CUSTOM JSON FILTER__ 
  * Check the __Audit authorization successes__ checkbox.
  ![customer-filter](img/custom-filter.png "custom-filter.png")

  * Paste in the filter in the *audit-filter.json* file in the repository or copy and pase it from the code snippet below.
  ```json
  {
    "$or": [
      {
  ...
  ```
  * Save the filter using the __Save__ button.

  This configured set of rules declares that any authenticated user who performs a set of DDL operations related to creating and destroying users, databases, collections, and indexes should have these specific activities captured in an audit trail. Additionally, DML ops related to `find`, `insert`, `delete`, `update`, and `findAndModify` will be captured as well.
...
* Create the database and collection by inserting a document
    ...
    ```
* Execute a find on the document that was inserted
  ```js
  db.new_coll.find({foo:"bar"})
  ```
* Execute an update on the inserted document
  ```js
  db.new_coll.updateOne({foo:"bar"}, {"$set": {"foo": "baz"}})
  ```
* Find the updated document
  ```js
  db.new_coll.find({foo:"baz"})
  ```
* Delete the inserted document
  ```js
   db.new_coll.deleteOne({foo: "baz"});
  ```
* Drop the collection
    ```js
    db.new_coll.drop()
