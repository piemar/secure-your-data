...
/*
  * Note: the Mapbox.js is no longer in active development and new API tokens will not be able to access
  * the resources exposed by that library. The access token here was taken from Carlos Castro who had
  * created it on his account prior to this change so should work with the older API
*/
// Replace with your Mapbox access token (or set via env)
const ACCESS_TOKEN = process.env.MAPBOX_ACCESS_TOKEN || 'YOUR_MAPBOX_ACCESS_TOKEN'
