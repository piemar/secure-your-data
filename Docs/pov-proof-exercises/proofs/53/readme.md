...
$ cd pov-proof-exercises/proofs/53/
$ python3 -m venv .ENV
...
2. Rename the `config.sample.py` file to `config.py` and replace the connection string defined within `config.py` with the connection string to your cluster.
...
6. Let's work with data and make use of window functions. For that we just import the aggregation pipeline into the cryptoTS collection from `agg_crypto.js` into compass. This pipeline generates the moving avg (50/200 day) of the close value. For getting results at the last stage you might need to deactivate the sample mode in the aggregation builder by going into settings changing the Number of Preview Documents & Limit values to 1508000.

![](img/agg1.png "Aggregation Pipeline Builder in Compass Part 1")
![](img/agg2.png "Aggregation Pipeline Builder in Compass Part 2")

7. Finally let's create a nice Visualization with Charts. To do so add your time series collection/cluster as data source in Charts. Then there are different ways to solve that. We choose to apply the aggregation pipeline from before directly on our data source. For that we:
- click at the datasource page
- choose the cluster you deployed the timeseries data
- expand upon the database
- select the Charts View button by the cryptoTS collection
- enter a name for the pipeline and copy/paste the code from the agg_crypto into the pipeline box
- click on the Test Pipeline Button to validate the results
- click on the Save Button to save the results

![](img/datasource.png "Add Pipeline to Data Source")

After applying the aggregation pipeline we can add the chart on a dashboard or import a `.charts`-dashboard file. If you want to import the .charts-file you can do so on the Charts Homescreen by clicking on the dropdown arrow by Add Dashboard and choosing Import dashboards. 
- Choose to connect now
- Specify the cluster & database you deployed the time series collection
- Instead of selecting the collection, choose the pipeline you just saved as the source for the chart. 
- Click the Save Button
- If the chart doesn't render correctly, follow the steps below to recreate it
...
-- select the time series collection with the aggregation pipeline as the data source
