...
1. **Pull the image**
   ```bash
   docker pull pierrepetersson/mongodb-workshop-sandbox-final:latest
   ```

2. **Run the container** (mount AWS config so the app can verify KMS)
   ```bash
   # Mac / Linux — AWS (default)
   Optiona 1: To force building locally and run app
   docker compose up app --build --force-recreate

   Option 2:  In @docker-compose.yml change image to pierrepetersson/mongodb-workshop-sandbox-final:latest
